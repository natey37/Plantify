...
const Gpt3Example = ({ user }) => {
  ...
  const handleGenerateText = async () => {
    try {
      setIsSaving(true);
      const generatedImage = await generateImageDalle({
        prompt: inputText,
        user: user,
      });
  ...
  const handleSavePlantClick = async () => {
    ...
    if (
      !currentCategories.includes(sanitizeAndPluralize(outputText["Category"]))
    ) {
      await createCategory({
        name: sanitizeAndPluralize(outputText["Category"]),
      });
    }
  ...
  return (
    <div className="gpt-container flex-column">
      ...
      {isSaving ? (
        ...
        <button className="submit-btn" onClick={handleGenerateText}>
          Generate Plant
        </button>
      )}
      {outputText && (
        <>
          <div className="plant-card">
            ...
            <p>
              ...
              {outputText["Fun Interesting Fact"]}
            </p>
            <img style={{ height: "auto", width: 500 }} src={generatedImage} />
          </div>
          <div className="button-row">
            <button className="submit-btn" onClick={handleSavePlantClick}>
              Save Plant
            </button>
            <button className="delete-btn" onClick={handleClearClick}>
              Clear
            </button>
          </div>
        </>
